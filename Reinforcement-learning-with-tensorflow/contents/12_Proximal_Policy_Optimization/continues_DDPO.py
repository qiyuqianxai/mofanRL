...
class a_net(nn.Module):
    def __init__(self,indim, outdim):
        ...
        self.mu = nn.Linear(200,outdim)
        self.sigma = nn.Linear(200,outdim)
    ...
    def forward(self,x):
        ...
        normal = torch.distributions.Normal(loc=mu*2,scale=sigma)
        return normal
...
class c_net(nn.Module):
    def __init__(self,indim):
        ...
        self.layer1 = nn.Linear(indim,100)
...
class PPO(object):
        ...
        while not Request_Stop:
            if GLOBAL_EP < self.args.EP_MAX:
                ...
                for i in range(self.args.UPDATE_STEP):
                    ...
                    a_c_normal = self.anet_c(b_s)
                    a_t_normal = self.anet_t(b_s)
                    ratio = torch.exp(a_c_normal.log_prob(b_a) - a_t_normal.log_prob(b_a))
                    # ratio = a_c_normal.log_prob(b_a).exp() / (a_t_normal.log_prob(b_a).exp() + 1e-5)
                    surr = ratio*adv
    ...
    def choose_action(self, s):  # run by a local
        ...
        self.anet_c.eval()
        with torch.no_grad():
            normal = self.anet_c(s)
            action = torch.squeeze(normal.sample(), dim=0).cpu().numpy()
    ...
    def work(self):
        ...
        while not Request_Stop:
            ...
            for t in range(self.args.EP_LEN):
                ...
                if t == self.args.EP_LEN - 1 or GLOBAL_UPDATE_COUNTER >= self.args.MIN_BATCH_SIZE:
            ...
            GLOBAL_EP += 1
            print("EP:",GLOBAL_EP,'{0:.1f}%'.format(GLOBAL_EP / self.args.EP_MAX * 100), '|W%i' % self.wid, '|Ep_r: %.2f' % ep_r, )
